...
class AtomtypesRecord(cp4vasp.AtomtypesRecord,AbInitio.vasp.parsing.Structure.AtomtypesRecord):
    ...
    def setAtomtypesRecord(self,a):
      ...
      elif isinstance(a,vasp.parsing.Structure.AtomtypesRecord):
	f=a.getFields();
	for x in self.getFields():
	  if x in f:
	    self[x]=a[x]
...
class AtomInfo(cp4vasp.AtomInfo,AbInitio.vasp.parsing.Structure.AtomInfo):
    ...
    def setAtomInfo(self,a):
      ...
      elif isinstance(a,vasp.parsing.Structure.AtomInfo):
        self.allocate(len(a))
...
#        print "cStructure.setAtomInfo",a
    ...
    def append(self,value):
      ...
      elif isinstance(value,vasp.parsing.Structure.AtomtypesRecord):
...
Performs value conversion, if necessary.    
    ...
    def readFromNode(self,elem):
      ...
      self.element=elem        
      a=vasp.parsing.Structure.AtomtypesArray(
        vasp.parsing.util.getChildrenByTagName(elem,"array",name="atomtypes")[0])
      self.allocate(len(a))
      for i in range(len(a)):
...
#        cp4vasp.Structure.__init__(self)
    ...
    def readFromNode(self,elem,atominfo=0):
      self.element=elem
      self.setStructure(vasp.parsing.Structure.Structure(elem,atominfo))
	...
	elif isinstance(s,vasp.parsing.Structure.Structure):
    ...
    __getmethods__.update({
        ...
        "info"                  : lambda x : AtomInfo(pointer=_cp4vasp.Structure_info_get(x)),
