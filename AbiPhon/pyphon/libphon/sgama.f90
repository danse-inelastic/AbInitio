...
SUBROUTINE sgama ( symm, latt, iprint )
  ...
  symm%is = 0
  nrot_cub = 48
  call cubicsym( latt%ats, symm%is, nrot_cub ) 
  symm%is = 0
  nrot_hex = 24
  call hexsym( latt%ats, symm%is, nrot_hex ) 
  ...
  if ( nrot == nrot_cub ) then
     ...
     call cubicsym( at, is, nrot ) 
  else
     if(iprint>0)print'(/''Found Hexagonal symmetry'')'
     call hexsym( at, is, nrot ) 
  endif
  ...
  do j = 2, natoms
     ...
     call checksym( is(:,:,1), ft, x, ityp, natoms, found )
     if ( found ) then
        trasl = trasl + 1
     endif
  enddo
  ...
  if(first) then
     ...
     do i = 1, natoms - 1
        do j = i+1, natoms
           do ntau = 2, trasl
              ft = x(:,j) - ftsuper(:,ntau)
              if( equiva( x(:,i), ft ) ) traslated(j) = .true.
           enddo
        enddo
     enddo
     ...
     enddo
!-------------------------------------------
! set the corrispondence supercell ---> cell
     allocate( latt%prim_atom(natoms) )
     do j = 1, nat
        do i = 1, natoms
           ft = x(:,i)*nsuper
           if( equiva( latt%x(:,j), ft ) ) latt%prim_atom(i) = j
        enddo
     enddo
!-------------------------------------------
  ...
  do isym = 1, nrot
     ...
     traslation: do i=1,natoms
        ...
        do j = 1, natoms
           ...
           !---------------------------------------------------------------------
           call checksym( symm%is(:,:,isym), ft, x, ityp, natoms, found )
...
!===================================================================
SUBROUTINE checksym ( is, ft, x, ityp, natoms, found )
  !===================================================================
  !
  ...
  REAL(DP) :: is(3,3), x(3,natoms), ft(3), rotx(3), eps(3)=1.d-6
