...
class VArray(TopArray):
  ...
  def __init__(self,data=[],t=FLOAT_TYPE,name=None,fastflag=0):
    """Create the VArray.
    *data* can be a python list, *VArray*, *UserList* or *Numeric* array instance,
    or XML element node.
...
#    self.element=data
  ...
  def readFromNodeFast(self,data):
    ...
    """
    try:
      import vasp.parsing.FArray as fa
...
class Array(TopArray):
  ...
  (use a[i][j][k], a[i,j,k] only works if converted to *Numeric* array.)
  ...
  def __init__(self,data=[],name=None,fastflag=0):
    """Create the Array.
    *data* can be a python list, *Array*, *UserList* or *Numeric* array instance,
    or XML element node.
