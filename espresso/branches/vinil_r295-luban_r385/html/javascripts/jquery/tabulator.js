...
(function($) {
  ...
  $.fn.sortable_column = function(colname) {
    ...
    var cell = this.find("td[colname=%s]" % colname);
	...
	thetable.sort_table_by_col( colname, direction );
  ...
  $.fn.sort_table_by_col = function( colname, direction ) {
    ...
    var newrows = sort_rows_by_col( saverows, colname, column_descriptors, direction );
  ...
  function new_row(rowid, rowindex, columnnames) {
    ...
    for (i=0; i<n; i++) {
      ...
      cell.attr( 'colname', columnnames[i]);
