...
Following calculation properties are available:
  ...
  ||KPOINT_TEXT             || string                                  || text of the KPOINTS file                         ||
  ...
  def readFromNode(self,elem):
    for x in elem.childNodes:
      if x.nodeName=="generation":
        self.generation=vasp.parsing.Dictionary.Dictionary(x)
...
class SystemPM(PropertyManager):
  def __init__(self,url=None):
    ...
    for name in ["POSCAR","POTCAR","INCAR","KPOINTS","CONTCAR"]:
...
class cStructureSequenceLateList(LateList):
  def __init__(self,plist,atominfo):
    LateList.__init__(self,plist)
    self.atominfo=vasp.parsing.cStructure.AtomInfo(atominfo)

  def parse(self,x):
    f=x.getElementsByTagName("structure")
    a=vasp.parsing.cStructure.Structure(pointer=cp4vasp.createStructure(f[0].this).this)
...
class XMLSystemPM(SystemPM):
  def __init__(self,url="vasprun.xml"):
    ...
    self.add("PATH",read=lambda x:getDirFromURL(x.manager.URL))
    self.add("DOM",
    ...
    self.add("KPOINT_TEXT",             read     =self.getKpointText)
    self.add("KPOINTS",
  ...
  def getEFermi(self,x):
	...
	d=open(x.manager.PATH+"DOSCAR","r")
	d.readline()
	d.readline()
	d.readline()
	d.readline()
...
#      eigenvalues =   projected.getElementsByTagName("eigenvalues")[0]
  ...
  def getInitialStructure(self,x):
    msg().status("Parsing initial structure")
    try:
      dom=x.manager.DOM
      atominfo=x.manager.ATOMINFO
      for e in dom.getElementsByTagName("structure"):
	if e.getAttribute("name")=="initialpos":
          s=Structure()
	  s.readFromNode(e,atominfo)
	  msg().status("OK")
          return s
    except:
      msg().error("Error - no initial structure available")
      msg().exception()
      raise
    msg().error("Error - no initial structure available")
  ...
  def getFinalStructure(self,x):
    msg().status("Parsing final structure")
    try:
      dom=x.manager.DOM
      atominfo=x.manager.ATOMINFO
      for e in dom.getElementsByTagName("structure"):
	if e.getAttribute("name")=="finalpos":
          s=Structure()
	  s.readFromNode(e,atominfo)
	  msg().status("OK")
          return s
    except:
      msg().error("Error - no final structure available")
      msg().exception()
      raise
    msg().error("Error - no final structure available")
...
class OldSystemPM(SystemPM):
  def __init__(self,url=None):
    ...
    self.add("PATH",read=lambda x:getDirFromURL(x.manager.URL))
    self.add("INCAR",
      read=lambda x:Incar(x.manager.PATH+"INCAR"))
    self.add("NAME",
      read=self.getName)
    self.add("INITIAL_STRUCTURE",
      read=lambda x:Structure(x.manager.PATH+"POSCAR"))
    self.add("FINAL_STRUCTURE",
      read=lambda x:Structure(x.manager.PATH+"CONTCAR"))
    self.add("FINAL_STRUCTURE",
      read=lambda x:Structure(x.manager.PATH+"CONTCAR"))
    self.add("TOTAL_DOS", read=self.getTotalDOS) 
  def getName(self,x):
...
class PoscarSystemPM(SystemPM):
  def __init__(self,path=None):
    ...
    self.add("PATH",read=lambda x:x.manager._geturl())
    self.add("INCAR",
...
class SystemPM_URL_Attribute(AttributeProfile):
  ...
  def setValue(self,obj,val):
    prop=obj["URL"]
    prop.value=val
    prop.status=vasp.parsing.Property.Property.READY
...
class SetupPM(SystemPM):
  def __init__(self,s=None):
    ...
    self.add("KPOINT_TEXT","Dummy k-points file\nGamma\n1 1 1\n0 0 0\n")
    self.inherit(s)
  def inherit(self,s=None):
    if s is not None:
      if s.INCAR is not None:
        self.INCAR=vasp.parsing.Dictionary.Incar(s.INCAR)	
      else:
        self.INCAR=vasp.parsing.Dictionary.Incar()
      self.INITIAL_STRUCTURE=vasp.parsing.Structure.Structure()
	...
	self.INITIAL_STRUCTURE.setStructure(s.INITIAL_STRUCTURE)
      if s.KPOINT_TEXT is not None:
	self.KPOINT_TEXT=s.KPOINT_TEXT
      else:
	self.KPOINT_TEXT="Dummy k-points file\nGamma\n1 1 1\n0 0 0\n"
    else:
      self.INCAR=vasp.parsing.Dictionary.Incar()
      self.INITIAL_STRUCTURE=vasp.parsing.Structure.Structure()
      self.KPOINT_TEXT="Dummy k-points file\nGamma\n1 1 1\n0 0 0\n"
