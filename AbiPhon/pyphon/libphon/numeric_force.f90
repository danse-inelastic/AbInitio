!========================================================================================
SUBROUTINE numeric_force( latt, symm, sh, dyn, iprint, nti )
  !======================================================================================
  ...
  LOGICAL :: found, equiv, equiva, equiva1
  ...
  REAL(DP), PARAMETER :: eps=1.d-8, large_disp = 0.1d0

  REAL(DP), ALLOCATABLE :: dis(:,:), force(:,:,:), force2(:,:,:), &
       force1(:,:,:), x(:,:), xtmp(:,:), xlatt_tmp(:,:), xlatt_tmp1(:,:), &
       tmpmat1(:,:,:), rmax(:), xws(:,:,:)

  REAL(DP) :: temp(3), temp1(3), tmp1(3), dx(3,3), invis(3,3), &
  ...
  do na = 1, latt%natoms
     do nb = 1, natoms
        if( equiva1( x(:,nb), latt%x(:,na) ) ) then
!-------------------------------------------------------------------
...
!--------------------------------------------------------------------
  ...
  do i=1,ndispl
     ...
     !========================================================================
     do na = 1, latt%natoms
        if( equiva1( x(:,adis(i)), latt%x(:,na) ) ) then
     ...
     !-----------------------------------------------
     ndis( adis(i) ) = ndis( adis(i) ) + 1
     disp_size = sqrt(dot_product(scale * matmul( at, dis(:,i) ),scale * matmul( at, dis(:,i) ))) 
     print'(''Atom '',i2,'', Displacement'',3f10.5,'', Size = '',f8.6'' (A)'')',adis(i),dis(:,i),&
          disp_size
  ...
  do naa = 1, latt%natoms
     ...
     if( ndis( naa ) > 0 )then
        ...
        do nbb = 1, latt%natoms
           ...
           enddo 
           if( .not. found ) stop 'Error, vector not found, try to increase RMAX'
        enddo
           ...
           do isym = 1, symm%nsymstart
              if( naa == symm%irt(nbb,isym) ) then
                 ...
                 call get_wig( latt, sh, natoms, scale, at, bg, x, &
                      xtmp, xlatt_tmp, nbb, rmax(naa), ws, xws ) 
                 wstot = wstot + sum(ws) 
                 do na = 1, natoms
                    if( ws(na) > 1 ) wsedge = wsedge + ws(na)
                 enddo
...
!                                if( equiva1( temp, xws(:,k,j) ) ) then
                                   ...
                                   donr: do nr = 1, sh%nrm
                                      ...
                                      do na = 1, latt%natoms
                                         ...
                                         if( equiva1( temp, tmp1 ) ) exit donr
                                      enddo
                                   enddo donr
                                   if( nr == sh%nrm + 1 ) &
                                        stop 'Error, nr not found, try to increase RMAX'
        ...
        do naa = 1, latt%natoms
           do nbb = 1, latt%natoms
              do nr = 1, sh%nrm
                 ...
                 if( vabs( temp ) > sh%cutoff ) then
                    print*,'azzero la matrice',naa,nbb,nr
                    print*, dyn%dmat(:,:,nbb,naa,nrr) 
                 ...
                 if( vabs( temp ) <= sh%rmax ) then
                    ...
                    do nrr = 1, sh%nrm
                       ...
                       if( vabs( tmp1 ) <= sh%rmax .and. equiva1( temp, tmp1 ) )then
