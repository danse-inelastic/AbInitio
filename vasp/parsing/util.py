...
try:
  import vasp.parsing.ODPdom as ODPdom
  use_odpdom=2
except:
  try:
    import ODPdom
...
#This was copied from __init__.py:
#beginning
INDENT        = "  "
INT_TYPE      = intern("int")
...
def isNumericArray(x):
  """Returns true if x is an *array* object from the *Numeric* library."""
  try:
...
def isArray(x):
  ...
  * *array* objects from the *Numeric* library.
...
def getAtomtypes():
  ...
  if atomtypes_ is not None:
    return atomtypes_      
  p=getPathChain()  
  atomtypes_=vasp.parsing.Structure.AtomInfo()
  for x in p:
    try:
      a=vasp.parsing.Structure.AtomInfo(x+sep+"atomtypes.xml")
